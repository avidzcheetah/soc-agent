...
| **EXP_004** | `EXP_20260712_004` | **Label Smoothing (Extended).** Same as EXP_002B but extended to 20 epochs with patience 4 to allow full convergence. | ❌ Macro F1: 0.614. Model overfit validation split. Stopped hyperparam tuning. |
| **EXP_005** | `(Pending)` | **CISA Phase Tags.** Base = EXP_003 (10 epochs, ls=0.1). Prepended CISA Phase (e.g. `[Detection]`) to all text inputs. | *(Pending)* |

*Note: Future experiments (e.g. Synthetic Augmentation) will continue this ablation sequence depending on the outcome of EXP_005.*
